...
**Atualização:** A partir da versão 2.3.0 do Flask, a variável de ambiente `FLASK_ENV` foi descontinuada e substituída pela variável `FLASK_DEBUG`. Desta forma, o comando de execução da aplicação, usando versões mais recentes do Flask, deve ficar assim:
